...
def process_ack(data):
    ...
    if type(data) != dict:
        data = json.loads(data)
    lamport_clock = max(data['lamport_clock'], lamport_clock)
    if ack_required != 0:
        ack_required -= 1
    if ack_required % 2 == 0:
        ...
        print(f"PID {PID}: {PID}.{top_evt['p_event_id']} has been executed after receiving all acks, lamports_clock {top_evt['lamport_clock']}")
        ...
        while True:
            for evt in reversed(event_queue[:]):
                ...
                for port in multicast_ports:
                    if port != my_port:
                        msg = ack_msg(PID, evt['p_event_id'])
...
def create_event():
    ...
    for port in multicast_ports:
        if port != my_port:
            msg = update_req_msg(PID, p_event_id, lamport_clock)
    ...
    msg_for_this_evt = update_req_msg(PID, p_event_id, lamport_clock)
